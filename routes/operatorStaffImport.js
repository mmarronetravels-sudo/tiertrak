...
async function validateStaffImport(req, res) {
  ...
  try {
    ...
    // Parse CSV. Each row is validated structurally; NO PII is echoed in
    // any error (§4B narrowing) — row number + field-level reason only.
    const parsePromise = new Promise((resolve, reject) => {
      fs.createReadStream(req.file.path)
        ...
        .on('data', (row) => {
          ...
          // always an operator, so canAssignRole returns true for every
          // role in ROLE_RANK; kept for symmetry with the commit importer
          // and as defense-in-depth if the gate ever changes.
          ...
          // school_wide_access consistency guard. Only fires when the
          // column was provided non-empty; omitted/empty is derived from
          // the role. Values here are not PII.
          ...
          // email value is NOT echoed (the commit importer does echo it;
          ...
          // district_id derivation for district-scoped roles is resolved
          // from the PATH districtId (verified non-null by the pre-flight),
          // not from the operator's own users.district_id — so the
          // operator-null footgun that the commit importer guards against
          // does not apply on this surface. No row error is raised here.
          results.push({ row: rowNumber, emailLower });
        })
        .on('end', () => resolve())
    ...
    // Mirrors the commit importer's cap.
    if (rowNumber - 1 > 100) {
      cleanup();
    ...
    for (const staff of results) {
      const existing = await pool.query(
        ...
        [staff.emailLower, schoolTenantId]
      );
...
module.exports = { upload, validateStaffImport };
