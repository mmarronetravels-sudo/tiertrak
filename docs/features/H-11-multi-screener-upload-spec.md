...
**Surface (Q4).** **Both** — school-admin self-serve **and** the operator console, sharing the row-processing core. The **school-admin** path (Slice B, `routes/screener.js`) binds tenant from the body via `resolveAndBindTargetTenant` → `resolveAccessibleTenantIds` (derived server-side). The **operator** path (Slice C, `routes/operatorScreenerImport.js`, mounted on the `platformAdminOnly` operator router) binds tenant **structurally from the URL path** (`:districtId/:schoolTenantId`) with district/school pre-flights, rejecting a school-not-in-district with **404** (probe-resistance, mirroring the operator student/staff importers). Both landed STAR-only with matched-only persistence.
