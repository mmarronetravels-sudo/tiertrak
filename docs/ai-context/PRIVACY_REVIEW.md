...
- **CSV / S3 / external egress**: no file uploads on the create path, and no student/referral data is ever transmitted to a third party. One external egress exists: on `POST /api/discipline-referrals/` the create path fires a post-COMMIT, fire-and-forget admin notification via Resend (`notifySchoolAdminsOfReferral`, reusing the existing inline Resend client + `RESEND_API_KEY` — same sub-processor already used by `routes/auth.js` and `routes/csvImport.js`, not a new vendor). What Resend receives: the recipient `school_admin` email address (staff PII) plus a deliberately **PII-free transactional body** — no student name, behavior, notes, grade, or referral id; the only link points at `FRONTEND_URL` root, never a referral-specific URL. Recipients are resolved by `SELECT email FROM users WHERE tenant_id = $1 AND role = 'school_admin'` where `$1` is the already-validated `targetTenantId` (never request input), and each recipient is mailed individually so no admin's address is disclosed to another. Failure path logs `tenant_id` (integer) + `err.name` only — never recipient addresses or row data. district_admin is intentionally NOT notified in v1 (recipients keyed on home `tenant_id`; cross-home-tenant / district-admin notification deferred — revisit alongside #122 cross-school onboarding).
...
**Per-tenant opt-out config (Migration 051).** `overdue_log_reminder_optouts` lets a school or a district decline the weekly email — the gate that must exist before `OVERDUE_LOGS_REMINDERS_ENABLED` is turned on in prod. Exactly one of (`school_tenant_id`, `district_id`) is set per row (`chk_optout_exactly_one_scope`); `reminders_enabled = FALSE` suppresses, absence of a row means eligible (default-on / opt-out semantics). The digest loads the declined scopes once per run as integer-only sets and skips them: a district-scoped row is applied as a per-USER skip (suppresses every school under that district, resting on the §5 `resolveAccessibleTenantIds` contract that a user's resolved schools all lie within `users.district_id`), a school-scoped row as a per-tenant skip. If the opt-out load fails the run aborts (fail-closed). The table stores integer references + a boolean + timestamps only (`school_tenant_id`, `district_id`, `reminders_enabled`, `created_by`, `updated_by`, `created_at`, `updated_at`) — **no student/staff names, no email addresses, no intervention data**; no foreign keys, consistent with the M031–M050 house style. The writers (district_admin self-service `PUT /api/districts/:id/overdue-log-reminders`; operator `PUT /api/operator/overdue-log-reminders/{schools/:tenantId,districts/:districtId}`) accept and echo integers + a boolean only — no PII in request, response, logs, or URL — and are rate-limited (`mutationUserLimiter`) under the global `/api` CSRF enforce. The new `skippedOptedOut` summary counter is **count-only and never carries a skipped user's id, email, or name**.

**School_admin self-service opt-out (PR `feat/overdue-logs-school-self-service-optout`).** A third writer reuses the same Migration-051 table with **no schema change**, letting a `school_admin` turn their OWN school's reminder on/off without an operator: `GET /api/school/overdue-log-reminders` returns the school's current `reminders_enabled` (absence of a row → `true`, default-on) plus a server-authoritative `feature_enabled` (whether `OVERDUE_LOGS_REMINDERS_ENABLED === 'true'`), and `PUT /api/school/overdue-log-reminders` upserts the school-scoped row (writing `updated_by` for an audit trail of who flipped it). Both are `school_admin`-ONLY — `district_tech_admin` (read-only by design) and `district_admin` (its own `/api/districts/:id/...` writer above) are excluded. §5: the target school is resolved strictly from `resolveAccessibleTenantIds(req.user)` (via the pure `routes/schoolOverdueLogOptoutsCore.resolveOwnSchoolId` helper, role-gate before input-parse, type-safe membership check) — never from request input, so a cross-school write is structurally impossible. Like the other two writers it accepts and echoes integers + a boolean only — **no PII in request, response, logs, or URL** — and the `PUT` is rate-limited (`mutationUserLimiter`) under the global `/api` CSRF enforce. The frontend `OverdueLogReminderToggle` card (Admin Panel, `school_admin` only) renders nothing until `feature_enabled` is true, so the control stays hidden until the email feature is live. The write is intentionally **not** flag-gated: a school may pre-declare its opt-out before the cron flag is turned on (the gate-item-(a) contract).

**Recipient-eligibility guard + send-burst controls (PR `sec/overdue-logs-recipient-guard`).** Before sending, each recipient passes an eligibility guard that excludes (a) operator accounts — `isOperator(user.id)` is consumed READ-ONLY against the frozen `PLATFORM_ADMIN_USER_IDS` allowlist, never modifying it, so a cross-tenant platform admin is never mailed any single tenant's overdue list — and (b) users with no usable email. (Departed staff are hard-deleted upstream and never appear; per-user deactivation has no schema column today and is a flagged follow-up.) Outbound calls are paced by `OVERDUE_LOGS_SEND_INTERVAL_MS` (per-send throttle) and bounded by `OVERDUE_LOGS_MAX_SENDS_PER_RUN` (per-run cap); a capped run leaves the unsent recipients' ledger slots unclaimed so a later tick within the same `week_of` re-sends them. The two summary counters this adds — `skippedIneligible` and `capReached` — are **count/boolean only and never carry a skipped user's id, email, or name**; the eligibility-skip branch logs nothing, and the cap-reached log line emits only the integer cap and sent count. No new PII surface, no new vendor, no new DB column.

**Calendar- & cadence-awareness (PR `feat/school-calendar-digest`, Migration 052).** The digest now skips a `(staffer, school)` for a week when the school is out of session, using the per-school academic calendar (`school_academic_calendar`, M052). (1) The digest reads that table scoped by `school_tenant_id` — `SELECT period_type, start_date, end_date FROM school_academic_calendar WHERE school_tenant_id = $1`, loaded lazily per processed school (`school_tenant_id` from the same `resolveAccessibleTenantIds(user)` resolution above, never request input) — and the `label` column is **deliberately NOT selected** (gate 5: the calendar `label` is convenience UI metadata that is never logged and never emailed). (2) The in-session decision is computed by the DB-free `services/schoolCalendar.js` from non-PII date/type fields only (`period_type`, `start_date`, `end_date`); it never reads `label` and never queries. (3) The new `skippedOutOfSession` summary counter is **count-only and never carries a user's id, email, or name**. The frequency-aware predicate change (`getMissingLogsForStaff` honoring `log_frequency` for `biweekly` cadence) adds no new column to the email body's SELECT — it is the same student/intervention row shape as before. No new table, no new PII surface, no new vendor.

**v1 exclusions / follow-ups.** Per-tenant opt-out (school + district) IS now built (Migration 051, above) — the gating follow-up is satisfied. Per-STAFF (per-user) opt-out is still NOT built and remains a follow-up. No school-name label is rendered in the per-school email body yet (acceptable while most users are single-school; revisit with the opt-out follow-up). Multi-instance correctness rests on the dedup ledger rather than leader election; if the Render deploy ever scales beyond one instance before the ledger is battle-tested, re-verify the claim-then-send path under concurrency.
