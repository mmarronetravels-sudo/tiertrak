...
const createPreReferralTable = async () => {
  try {
    ...
    -- Migration 008: MTSS Meetings with Intervention Review
-- Created: January 27, 2026
-- Purpose: Replace free-form MTSS notes with structured meeting forms
...
CREATE TABLE IF NOT EXISTS mtss_meetings (
  ...
  meeting_number INTEGER DEFAULT 1 CHECK (meeting_number IN (1, 2, 3)), -- 1st, 2nd, or 3rd meeting in cycle
...
-- Intervention evaluations for each meeting
CREATE TABLE IF NOT EXISTS mtss_meeting_interventions (
  ...
  -- Store snapshot of progress data at time of meeting
...
CREATE INDEX IF NOT EXISTS idx_mtss_meetings_date ON mtss_meetings(meeting_date);
CREATE INDEX IF NOT EXISTS idx_mtss_meeting_interventions_meeting ON mtss_meeting_interventions(mtss_meeting_id);

-- Note: Existing progress_notes table is kept for legacy data
-- Old notes will still be displayed but new meetings use this structured format  } catch (error) {
